...
export const checkProfile = async (displayName) => {
    try {
        const userRef = ref(db, `users/${displayName}`);
        const userSnapshot = await get(userRef);
        
        if (!userSnapshot.exists()) {
            await set(userRef, { trips: {} });
            return { trips: {} };
        }
...
// return trip
export const getTrip = async (displayName, tripName) => {
    try {
        const tripRef = ref(db, `users/${displayName}/trips/${tripName}`);
